...
export class DataProcessorRunnable extends Runnable {
  ...
  async *invoke(input, context) {
    ...
    try {
      ...
      if (!Array.isArray(input)) {
        yield fatal('Invalid input: expected array', null, { inputType: typeof input });
      ...
      const { result: finalResult, event: validationEvent } = await measureAsync(
        'Final Validation',
  ...
  async processBatch(batch, batchNumber) {
    ...
    for (let i = 0; i < batch.length; i++) {
      ...
      try {
        ...
        if (item === null || item === undefined) {
          events.push(warning(`Skipping null/undefined item at batch ${batchNumber}, index ${i}`));
          skipped++;
          continue;
        }
        ...
        // Log slow items
        const lastDuration = itemTimer.measurements[itemTimer.measurements.length - 1];
        if (lastDuration > 8) {
          events.push(warning(`Slow item processing detected`, {
            item: item,
        ...
        events.push(error(`Failed to process item at batch ${batchNumber}, index ${i}`, itemError, {
          item: item,
          batchNumber,
          itemIndex: i
        }));
    ...
    if (itemTimer.measurements.length > 0) {
      events.push(itemTimer.performanceStats({
        batchNumber,
    ...
    while (attempts < this.maxRetries) {
      try {
        ...
        if (attempts >= this.maxRetries) {
          throw new Error(`Failed to process item after ${this.maxRetries} attempts: ${err.message}`);
        }
...
export async function runDataProcessorExample() {
  ...
  const sampleData = [
    ...
    { id: 2, name: 'Bob' },
    4, 5, 6, 'test', undefined, // Will be skipped
    { id: 3, name: 'Charlie' }
  ];
  ...
  try {
    ...
    while (!done) {
      ...
      if (!done) {
        ...
        if (event.type === 'log') {
          console.log(`[${timestamp}] ${event.level.toUpperCase()}: ${event.message}`);
          if (event.error) {
          ...
          if (event.statistics) {
            console.log(`  Stats: ${JSON.stringify(event.statistics, null, 2)}`);
          }
        }
      } else {
        result = value;
