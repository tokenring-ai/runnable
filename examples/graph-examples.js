...
export async function fanOutFanInPattern() {
    ...
    const personalProcessor = new TransformRunnable('PersonalProcessor', (input) => ({
        ...input,
        fullName: input.name.toUpperCase(),
        ageGroup: input.age < 18 ? 'minor' : input.age < 65 ? 'adult' : 'senior'
    }));
    
    const contactProcessor = new TransformRunnable('ContactProcessor', (input) => ({
        ...input,
        emailDomain: input.email ? input.email.split('@')[1] : null,
        hasPhone: !!input.phone
    }));
    
    const metadataProcessor = new TransformRunnable('MetadataProcessor', (input) => ({
        ...input,
    ...
    const pipeline = RunnableGraph.builder({ 
        ...
        .node('splitter', splitter, { outputs: ['personalInfo', 'contactInfo', 'metadata'] })
        .node('personalProcessor', personalProcessor)
        .node('contactProcessor', contactProcessor)
        .node('metadataProcessor', metadataProcessor)
        .node('aggregator', aggregator, { inputs: ['personal', 'contact', 'metadata'] })
...
export async function conditionalProcessingWithErrorHandling() {
    ...
    const typeAProcessor = new TransformRunnable('TypeAProcessor', (input) => {
        ...
        return { ...input, processedBy: 'TypeAProcessor', result: input.data.toUpperCase() };
    });
    
    const typeBProcessor = new TransformRunnable('TypeBProcessor', (input) => {
        if (input.type !== 'B') throw new Error('Not type B');
        return { ...input, processedBy: 'TypeBProcessor', result: input.data.toLowerCase() };
    });
    
    const fallbackProcessor = new TransformRunnable('FallbackProcessor', (input) => ({
        ...input,
...
export async function realWorldDataProcessingPipeline() {
    ...
    const inventoryChecker = new TransformRunnable('InventoryChecker', async (input) => {
    ...
    const paymentProcessor = new TransformRunnable('PaymentProcessor', async (input) => {
    ...
    const notificationSender = new TransformRunnable('NotificationSender', async (input) => {
    ...
    const pipeline = RunnableGraph.builder({ 
        ...
        .node('finalizer', orderFinalizer, { inputs: ['fulfillment', 'notification'] })
