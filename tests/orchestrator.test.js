...
class MockRunnable extends Runnable {
  ...
  async *invoke(input) {
    yield { type: 'log', level: 'info', message: `MockRunnable '${this.name}' invoked`, timestamp: Date.now(), runnableName: this.name };
    ...
    yield { type: 'log', level: 'info', message: `Completed execution of node '${this.name}'`, timestamp: Date.now(), runnableName: this.name };
    return output;
  }
}
...
describe('GraphOrchestrator', () => {
  it('dynamically adds nodes and skips completed ones', async () => {
    ...
    class DynamicOrchestrator extends GraphOrchestrator {
      async updateGraph({ graph, persistence }) {
        if (persistence.completedNodes.includes('n2') && !graph.describe().nodes.includes('n3')) {
    ...
    const started = events.filter(e => e.message?.startsWith('Starting execution')).map(e => e.nodeId);
  ...
  it('resumes using persistence', async () => {
    ...
    const { events: ev2, returnValue } = await consumeGenerator(orch.invoke(0, { persistence: state }));
    expect(returnValue).toBe(3);
    const started = ev2.filter(e => e.message?.startsWith('Starting execution')).map(e => e.nodeId);
