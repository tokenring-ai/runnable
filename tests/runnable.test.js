...
describe('Runnable (core/runnable/runnable.js)', () => {
    ...
    describe('Base invoke() method (placeholder behavior)', () => {
        it('should yield a warning log event and return the input', async () => {
            ...
            const { events, returnValue } = await consumeGenerator(runnable.invoke(input, context));
            ...
            expect(logEvent).toMatchObject({
                type: 'log',
                level: 'warn',
                message: 'Base invoke for BaseInvokeTest. Input will be returned as output.',
        ...
        it('should work with an undefined name', async () => {
            ...
            const { events, returnValue } = await consumeGenerator(runnable.invoke(input, {}));

            expect(consoleWarnSpy).toHaveBeenCalledWith(
                "Runnable: invoke() method not implemented. Defaulting to no-op pass-through."
            );
            expect(events[0].message).toBe('Base invoke for Unnamed Runnable. Input will be returned as output.');
    ...
    describe('validation behavior', () => {
        it('should validate input when schema provided', async () => {
            const runnable = new Runnable({ inputSchema: z.object({ x: z.number() }) });
            const { events, returnValue } = await consumeGenerator(runnable.invoke({ x: 'bad' }));
        ...
        it('should bypass input validation when disabled', async () => {
            const runnable = new Runnable({ inputSchema: z.object({ x: z.number() }), validateInput: false });
            const { returnValue } = await consumeGenerator(runnable.invoke({ x: 'skip' }));
