...
async function consumeGenerator(generator) {
    ...
    try {
        ...
        events.push({ type: 'test_framework_error_catch', error: { name: e.name, message: e.message } });
        throw e;
    }
    ...
    async *invoke(input, context) {
        ...
        if (this.behavior.delay > 0) {
            ...
            while (Date.now() - startTime < this.behavior.delay) {
                ...
                await new Promise(resolve => setTimeout(resolve, Math.min(10, this.behavior.delay)));
            }
        }
...
describe('RunnableGraph', () => {
    ...
    describe('Node Management', () => {
        ...
        it('should add nodes successfully', () => {
            ...
            expect(() => graph.addNode('node2', new MockRunnable('test2'))).not.toThrow();
        });
        ...
        it('should throw error when adding duplicate node IDs', () => {
            ...
            expect(() => graph.addNode('node1', new MockRunnable('test2')))
                .toThrow("Node with id 'node1' already exists");
        });

        it('should throw error when adding non-Runnable instance', () => {
            expect(() => graph.addNode('node1', {}))
                .toThrow("Node 'node1' must be a Runnable instance");
        });

        it('should accept node configuration', () => {
            const runnable = new MockRunnable('test');
            expect(() => graph.addNode('node1', runnable, {
    ...
    describe('Entry and Exit Nodes', () => {
        ...
        it('should throw error when setting non-existent entry node', () => {
            expect(() => graph.setEntryNodes('nonexistent'))
                .toThrow("Entry node 'nonexistent' does not exist");
        });

        it('should throw error when setting non-existent exit node', () => {
            expect(() => graph.setExitNodes('nonexistent'))
                .toThrow("Exit node 'nonexistent' does not exist");
        });
    });
    ...
    describe('Graph Validation', () => {
        it('should throw error for empty graph', async () => {
            ...
            await expect(consumeGenerator(generator)).rejects.toThrow('Graph must contain at least one node');
        });
        ...
        it('should throw error for graph without entry nodes', async () => {
            ...
            const generator = graph.invoke('test input');
            await expect(consumeGenerator(generator)).rejects.toThrow('Graph must have at least one entry node');
        });
        ...
        it('should throw error for graph without exit nodes', async () => {
            ...
            const generator = graph.invoke('test input');
            await expect(consumeGenerator(generator)).rejects.toThrow('Graph must have at least one exit node');
        });
    });
    ...
    describe('Simple Graph Execution', () => {
        it('should execute single node graph', async () => {
            ...
            graph.addNode('single', mockRunnable)
                 .setEntryNodes('single')
                 .setExitNodes('single');

            const { events, returnValue } = await consumeGenerator(graph.invoke('test input'));

            expect(returnValue).toBe('processed');
            
            // Check for graph start/end events
            const graphStartEvent = events.find(e => e.message?.includes('Starting graph execution'));
            const graphEndEvent = events.find(e => e.message?.includes('Graph execution completed'));
        ...
        it('should execute linear chain of nodes', async () => {
            ...
            const { events, returnValue } = await consumeGenerator(graph.invoke('initial'));

            expect(returnValue).toBe('final');
            
            // Verify execution order through events
            const nodeStartEvents = events.filter(e => e.message?.includes('Starting execution of node'));
    ...
    describe('Multi-Input/Output Scenarios', () => {
        ...
        it('should handle multi-output nodes', async () => {
            ...
            graph.addNode('splitter', splitter, { outputs: ['output1', 'output2'] })
                 .addNode('consumer1', consumer1, { inputs: ['input'] })
                 .addNode('consumer2', consumer2, { inputs: ['input'] })
                 .connect('splitter', 'consumer1', { fromOutput: 'output1', toInput: 'input' })
                 .connect('splitter', 'consumer2', { fromOutput: 'output2', toInput: 'input' })
    ...
    describe('Error Handling', () => {
        it('should propagate errors from failed nodes', async () => {
            ...
            graph.addNode('failing', failingNode)
                 .setEntryNodes('failing')
                 .setExitNodes('failing');

            const generator = graph.invoke('test input');
            await expect(consumeGenerator(generator)).rejects.toThrow('MockRunnable failing failed as configured');
        });
        ...
        it('should handle abort signals', async () => {
            ...
            const slowNode = new MockRunnable('slow', { delay: 1000 }, { abortController });
            graph.addNode('slow', slowNode)
                 .setEntryNodes('slow')
                 .setExitNodes('slow');
        ...
        it('should report dependency details when execution gets stuck', async () => {
            ...
            await expect(consumeGenerator(generator)).rejects.toThrow(/Remaining nodes: orphan waiting for need/);
        });
    });
    ...
    describe('Event Propagation', () => {
        it('should enhance events with node and graph context', async () => {
            ...
            graph.addNode('eventNode', eventNode)
            ...
            const debugEvent = events.find(e => e.type === 'log' && e.level === 'debug');
    ...
    describe('describe()', () => {
        it('should return a summary of the graph', () => {
            ...
            expect(summary).toEqual({
                nodes: ['node1', 'node2'],
                connections: [
                    { from: 'node1', to: 'node2', fromOutput: 'output', toInput: 'input' }
                ],
                entryNodes: ['node1'],
                exitNodes: ['node2'],
                options: {
...
describe('Graph Persistence', () => {
    it('should resume execution with saved state', async () => {
        ...
        while (!r.done) {
            if (r.value.message && r.value.message.includes("Completed execution of node 'n2'")) {
                await gen1.return();
                break;
            }
            r = await gen1.next();
        ...
        const { events, returnValue } = await consumeGenerator(graph.invoke(0, { persistence: state }));
        expect(returnValue).toBe(3);
        const startEvents = events.filter(e => e.message?.startsWith('Starting execution of node'));
...
describe('Integration Tests', () => {
    it('should handle complex real-world-like scenario', async () => {
        ...
        const graph = RunnableGraph.builder({ name: 'DataPipeline', parallel: true })
        ...
        const nodeExecutions = events.filter(e => e.message?.includes('Starting execution of node'));
        expect(nodeExecutions).toHaveLength(6);
        
        // Verify graph completion
        const completionEvent = events.find(e => e.message?.includes('Graph execution completed'));
...
describe('RunnableGraph Schema Validation', () => {
    ...
    describe('Schema Presence Validation', () => {
        it('should warn when nodes have no schemas', () => {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith('Graph schema validation warnings:');
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining("Node 'A' input has no schema defined"));
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining("Node 'A' output has no schema defined"));
    ...
    describe('Compatible Schema Validation', () => {
        it('should pass validation with compatible schemas', () => {
            const nodeA = createTestRunnable('A', 
                z.object({ value: z.number() }), 
                z.object({ result: z.string() })
            );
            const nodeB = createTestRunnable('B', 
                z.object({ result: z.string() }), 
    ...
    describe('Incompatible Schema Validation', () => {
        it('should throw error for incompatible basic types', () => {
            ...
            expect(() => graph.connect('A', 'B')).toThrow(/Schema incompatibility between 'A' \(output\) and 'B' \(input\)/);
        });
        ...
        it('should throw error for missing required properties', () => {
            ...
            // Error should be thrown during graph construction, not during invoke
            expect(() => graph.connect('A', 'B')).toThrow(/Required input property 'required' is not provided/);
        });
        ...
        it('should throw error for nullable output to non-nullable input', () => {
            ...
            // Error should be thrown during graph construction, not during invoke
            expect(() => graph.connect('A', 'B')).toThrow(/Output can be null but input does not accept null/);
        });
    });
    ...
    describe('Optional Schema Validation', () => {
        it('should warn about optional output to required input', () => {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith('Graph schema validation warnings:');
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Output is optional but input is required'));
        ...
        it('should warn about missing optional properties', () => {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith('Graph schema validation warnings:');
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining("Optional input property 'optional' is not provided"));
    ...
    describe('Multi-output Node Validation', () => {
        it('should warn about multi-output node validation limitations', () => {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith('Graph schema validation warnings:');
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining("Cannot validate specific output 'output1' from multi-output node"));
