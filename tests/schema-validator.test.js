...
import { z } from 'zod';
import { validateZodTypeCompatibility, validateSchemaExists } from '../schema-validator.js';
...
describe('Schema Validator', () => {
  describe('validateSchemaExists', () => {
    it('should return warning when schema is missing', () => {
      ...
      expect(result.warnings[0]).toContain('test context has no schema defined');
  ...
  describe('validateZodTypeCompatibility - Optional and Nullable', () => {
    it('should warn when output is optional but input is required', () => {
      ...
      expect(result.compatible).toBe(true);
      expect(result.warnings).toHaveLength(1);
      expect(result.warnings[0]).toContain('Output is optional but input is required');
    });
    ...
    it('should error when output is nullable but input is not', () => {
      ...
      expect(result.compatible).toBe(false);
      expect(result.errors).toHaveLength(1);
      expect(result.errors[0]).toContain('Output can be null but input does not accept null');
    });
  ...
  describe('validateZodTypeCompatibility - Object Types', () => {
    ...
    it('should detect missing required properties', () => {
      ...
      expect(result.compatible).toBe(false);
      expect(result.errors).toHaveLength(1);
      expect(result.errors[0]).toContain("Required input property 'age' is not provided");
    });
    ...
    it('should warn about missing optional properties', () => {
      ...
      expect(result.compatible).toBe(true);
      expect(result.warnings).toHaveLength(1);
      expect(result.warnings[0]).toContain("Optional input property 'age' is not provided");
    });
    ...
    it('should validate nested object properties', () => {
      ...
      expect(result.compatible).toBe(false);
      expect(result.errors.some(e => e.includes("Property 'user'"))).toBe(true);
    });
  });
  ...
  describe('validateZodTypeCompatibility - Union Types', () => {
    ...
    it('should find compatible paths in union types', () => {
      ...
      expect(result.compatible).toBe(true);
      expect(result.warnings.some(w => w.includes('compatible option'))).toBe(true);
    });
    ...
    it('should detect incompatible union types', () => {
      ...
      expect(result.compatible).toBe(false);
      expect(result.errors.some(e => e.includes('No compatible option'))).toBe(true);
    });
  });
  ...
  describe('validateZodTypeCompatibility - Enum Types', () => {
    ...
    it('should detect incompatible enum types', () => {
      ...
      expect(result.compatible).toBe(false);
      expect(result.errors.some(e => e.includes('no common values'))).toBe(true);
    });
    ...
    it('should warn about partially overlapping enum types', () => {
      ...
      expect(result.compatible).toBe(true);
      expect(result.warnings.some(w => w.includes('different sets of values'))).toBe(true);
    });
  });
  ...
  describe('validateZodTypeCompatibility - Literal Types', () => {
    ...
    it('should detect incompatible literal types', () => {
      ...
      expect(result.compatible).toBe(false);
      expect(result.errors.some(e => e.includes("Literal values don't match"))).toBe(true);
    });
  });
  ...
  describe('validateZodTypeCompatibility - Edge Cases', () => {
    ...
    it('should handle void/undefined output types', () => {
      ...
      expect(result.compatible).toBe(false);
      expect(result.errors.some(e => e.includes('void/undefined but input is required'))).toBe(true);
    });
