...
function validateObjectCompatibility(outputSchema, inputSchema) {
	...
	for (const [inputKey, inputProp] of Object.entries(inputSchema.properties)) {
		...
		result.warnings.push(
			...propResult.warnings.map((w) => `Property '${inputKey}': ${w}`),
		);
		result.errors.push(
			...propResult.errors.map((e) => `Property '${inputKey}': ${e}`),
		);
...
function validateArrayCompatibility(outputSchema, inputSchema) {
	...
	if (!outputSchema.element || !inputSchema.element) {
		result.warnings.push(
			"Array element schemas not available for detailed validation",
		);
		return result;
	}

	// Validate element compatibility
	const elementResult = validateSchemaInfoCompatibility(
		outputSchema.element,
		inputSchema.element,
	);
	result.warnings.push(
		...elementResult.warnings.map((w) => `Array element: ${w}`),
	);
	...
	if (outputSchema.type === "union" && inputSchema.type === "union") {
		...
		for (const outputOption of outputSchema.union) {
			for (const inputOption of inputSchema.union) {
				const optionResult = validateSchemaInfoCompatibility(
					outputOption,
					inputOption,
				);
		...
		for (const outputOption of outputSchema.union) {
			...
			if (optionResult.compatible) {
				result.compatible = true;
				result.warnings.push(
					"Union output type has compatible option for input",
				);
				return result;
			}
		}
		...
		for (const inputOption of inputSchema.union) {
			...
			if (optionResult.compatible) {
				result.compatible = true;
				result.warnings.push(
					"Output type is compatible with at least one union input option",
				);
				return result;
			}
		}
		result.errors.push(
			"Output type is not compatible with any union input option",
		);
	}
...
function validateSchemaInfoCompatibility(outputSchema, inputSchema) {
	...
	if (outputSchema.nullable && !inputSchema.nullable && !inputSchema.optional) {
		result.errors.push(
			"Output can be null but input does not accept null values",
		);
		result.compatible = false;
	}
	...
	if (outputSchema.optional && !inputSchema.optional) {
		result.warnings.push(
			"Output is optional but input is required - may cause runtime errors if output is undefined",
		);
	}
	...
	if (outputSchema.type === "enum" && inputSchema.type === "enum") {
		...
		if (!hasCommonValues) {
			...
			![...outputValues].every((v) => inputValues.has(v))
		) {
			result.warnings.push(
				"Enum types have different sets of values but some overlap",
			);
		}
		return result;
	}
	...
	if (outputSchema.type === "literal" && inputSchema.type === "literal") {
		if (outputSchema.literal !== inputSchema.literal) {
			result.errors.push(
				`Literal values don't match: ${outputSchema.literal} !== ${inputSchema.literal}`,
			);
			result.compatible = false;
		}
		return result;
	...
	if (!areBasicTypesCompatible(outputSchema.type, inputSchema.type)) {
		result.errors.push(
			`Incompatible types: ${outputSchema.type} cannot be used as ${inputSchema.type}`,
		);
		result.compatible = false;
		return result;
	}
