...
export class RunnableGraph extends Runnable {
  ...
  #validateConnectionSchema(fromNodeId, toNodeId, fromOutput, toInput) {
    ...
    if (fromOutput !== 'output' && fromNode.outputs.length > 1) {
      ...
      console.warn(`  - Cannot validate specific output '${fromOutput}' from multi-output node '${fromNodeId}' - using full output schema`);
      return; // Skip validation for multi-output nodes
    }

    const compatibilityResult = validateZodTypeCompatibility(actualFromSchema, toSchema);
    
    // Log warnings if any (but don't throw)
    if (compatibilityResult.warnings.length > 0) {
      console.warn('Graph schema validation warnings:');
      compatibilityResult.warnings.forEach(warning => 
        console.warn(`  - Connection '${fromNodeId}' → '${toNodeId}': ${warning}`)
      );
    }
  ...
  #validateAllSchemas() {
    ...
    for (const [nodeId, node] of this.#nodes) {
      const inputResult = validateSchemaExists(node.runnable.inputSchema, `Node '${nodeId}' input`);
      const outputResult = validateSchemaExists(node.runnable.outputSchema, `Node '${nodeId}' output`);
    ...
    for (const edge of this.#edges) {
      ...
      if (edge.fromOutput !== 'output' && fromNode.outputs.length > 1) {
        ...
        // This is a limitation - we can't easily validate specific output keys without
        // more sophisticated schema introspection
        warnings.push(`Cannot validate specific output '${edge.fromOutput}' from multi-output node '${edge.from}' - using full output schema`);
      ...
      warnings.push(...compatibilityResult.warnings.map(w => 
        `Connection '${edge.from}' → '${edge.to}': ${w}`
      ));
      
      // Only add to errors if actually incompatible
      if (!compatibilityResult.compatible) {
        errors.push(`Schema incompatibility between '${edge.from}' (output) and '${edge.to}' (input): ${compatibilityResult.errors.join(', ')}`);
      }
    }
    ...
    if (errors.length > 0) {
      const errorMessage = 'Graph schema validation failed:\n' + 
  ...
  #getReadyNodes(context) {
    ...
    for (const [nodeId, node] of this.#nodes) {
      ...
      if (context.completedNodes.has(nodeId) || context.failedNodes.has(nodeId)) {
        continue;
      }
  ...
  #prepareNodeInput(node, context) {
    ...
    if (node.inputs.length === 1) {
      ...
      if (mapping) {
        ...
        return sourceOutputKey === 'output' ? sourceResult : context.nodeOutputs.get(`${sourceNodeId}.${sourceOutputKey}`);
      }
      return undefined;
    }
    ...
    for (const inputKey of node.inputs) {
      ...
      if (mapping) {
        const [sourceNodeId, sourceOutputKey] = mapping.split('.');
        const sourceResult = context.nodeResults.get(sourceNodeId);
        inputData[inputKey] = sourceOutputKey === 'output' ? sourceResult : context.nodeOutputs.get(`${sourceNodeId}.${sourceOutputKey}`);
      }
    }
    return inputData;
  }
      ...
      if (node.outputs.length > 1 && result && typeof result === 'object') {
        for (const outputKey of node.outputs) {
          if (result[outputKey] !== undefined) {
            context.nodeOutputs.set(`${nodeId}.${outputKey}`, result[outputKey]);
            context.persistence.nodeOutputs[`${nodeId}.${outputKey}`] = result[outputKey];
          }
        }
      }
  ...
  async *invoke(input, context = {}) {
    ...
    try {
      ...
      while (executionContext.completedNodes.size + executionContext.failedNodes.size < this.#nodes.size) {
        ...
        if (readyNodes.length === 0) {
          ...
          const remainingNodes = Array.from(this.#nodes.keys()).filter(
            nodeId => !executionContext.completedNodes.has(nodeId) && !executionContext.failedNodes.has(nodeId)
          );
        ...
        if (this.#options.parallel && readyNodes.length > 1) {
          ...
          const concurrency = Math.min(readyNodes.length, this.#options.maxConcurrency);
          const nodePromises = readyNodes.slice(0, concurrency).map(async (nodeId) => {
            const events = [];
      ...
      executionContext.persistence.completedNodes = Array.from(executionContext.completedNodes);
      executionContext.persistence.failedNodes = Array.from(executionContext.failedNodes);
      executionContext.persistence.nodeResults = Object.fromEntries(executionContext.nodeResults);
      executionContext.persistence.nodeOutputs = Object.fromEntries(executionContext.nodeOutputs);
      executionContext.persistence.nodeErrors = Object.fromEntries(executionContext.nodeErrors);
